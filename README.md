...
<details>
<summary><b>🌐 Adding a language</b> (already 23 — unlikely to be needed, but here is the recipe)</summary>
...
3. If `api.fifa.com` serves the language, add it to `LANGS` in `scripts/update.mjs`; otherwise add it to `CLDR_LANGS` there.
4. Translate the curated bits: 16 `rainNote` entries (`climate.json`), 90 broadcaster notes (`broadcasters.json`), the SF Bay Area label (`Venues.tsx`), and 16 city names (`city-l10n.json`, non-Latin scripts only).
